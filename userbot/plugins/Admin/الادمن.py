...
async def promote(promt):
    ...
    try:
        ...
        await icse.edit("** ᥀︙تم تـرقيتـه مشـرف.**")
...
async def promote(promt):
    ...
    icse = await eor(promt, "**◄ جـاري رفعه مالك.**")
    user, rank = await get_user_from_event(promt)
    if not rank:
        rank = "مالك"
    if not user:
    ...
    try:
        await promt.client(EditAdminRequest(promt.chat_id, user.id, new_rights, rank))
        await icse.edit("** ᥀︙تم تـرقيتـه مالك.**")
    except BadRequestError:
        await icse.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    try:
        ...
        await icse.edit(NO_PERM)
        return
    await icse.edit("**◄ تم تنزيله من الاشراف بنحاح .**")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    icse = await eor(bon, "**◄ جاري حظر العضو .**")
    try:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    icse = await eor(unbon, "**◄ جاري رفع القيود .**")
    ...
    try:
        ...
        await icse.edit("** ᥀︙تم رفـع جميـع القيـود بنجـاح .**")
...
async def startmute(event):
    ...
    if event.is_private:
        ...
        if not user:
            return
        if user.id == bot.uid:
            return await eor(event, "** ᥀︙لا استـطيع تقيـد نفـسي .**")
        ...
        try:
            ...
            if not admin and not creator:
                await eor(
                    event, "** ◄ لا استطيع تقـيد شخص بـدون صلاحـيات الشرفـين.** "
                )
                return
...
async def endmute(event):
    ...
    if event.is_private:
        ...
        try:
            ...
            await event.edit("**  ᥀︙تم رفـع القيـود عـنه .**")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "#رفع_القيود\n"
        ...
        try:
            ...
            return await eor(event, f"**خطأ : **`{str(e)}`")
        await eor(event, "**  ᥀︙تم رفـع القيـود عـنه .**")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "#رفع_القيود\n"
