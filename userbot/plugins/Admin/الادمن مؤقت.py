...
@bot.on(admin_cmd(pattern=r"اكتم(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern=r"اكتم(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def tmuter(kimo):
    ...
    if not admin and not creator:
        ...
        return
    icse = await edit_or_reply(kimo, "◄ جاري كتم المستخدم .")
    ...
    if user.id == self_user.id:
        await icse.edit(f"**◄ لا استطيـع كتم نفسـي.**")
        return
    try:
        await icse.client(
            EditBannedRequest(
...
@bot.on(admin_cmd(pattern="احظر(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="احظر(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(kimo):
    chat = await kimo.get_chat()
    admin = chat.admin_rights
    ...
    if not admin and not creator:
        await edit_or_reply(kimo, NO_ADMIN)
        return
    icse = await edit_or_reply(kimo, "◄ جاري الحظر .")
    user, reason = await get_user_from_event(kimo, icse)
    if not user:
        return
    if reason:
    ...
    if not itime:
        ...
        return
    if user.id == self_user.id:
        await icse.edit(f"**◄ لا استطيـع حظر نفسـي.**")
        return
