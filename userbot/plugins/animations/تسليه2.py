# animation code for @EgyThon edit by @JAI6H 
...
@icssbot.on(admin_cmd(pattern=r"ضحك$"))
@icssbot.on(sudo_cmd(pattern="ضحك$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    event = await eor(event, ".🤣")
...
@icssbot.on(admin_cmd(pattern=r"مبضون$"))
@icssbot.on(sudo_cmd(pattern="مبضون$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    event = await eor(event, "🙂.")
