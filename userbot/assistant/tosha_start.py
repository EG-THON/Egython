...
# =================== OWNER - Ralls =================== #
ALIVE_NAME = Config.ALIVE_NAME if Config.ALIVE_NAME else "@Egython"
...
async def owner(event):
    await event.reply(startowner,
                             buttons=[
                                 [Button.inline(
                                     "• الاعدادات •", data="settings"),
                                  Button.inline(
                                     "• الزغرفه •", data="icszag")],
                                 [Button.inline("• الاذاعه •",
                                                data="toshbroad")],
                                 [Button.url("• الدعم •",
                                             "https://t.me/Egython")]
                             ])
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        buttons=[
            ...
            [Button.url("💠 𝘾𝙍𝘼𝙎𝙃𝙀𝘿 💠", "t.me/Egython")]
        ])
...
async def settings(event):
    ...
    await tgbot.send_message(event.chat_id,
                             ...
                             buttons=[
                                     [Button.inline(
                                         "• احصل على بوت ايجيثون بنفسك •", data="deployme")]
                             ])
...
async def settings(event):
    await event.delete()
    await event.reply(f" ᥀︙انا ايجيثون مساعد {mention}. الشخصي تستطيع الاتصال بي عن طريق هذا البوت",
                             buttons=[
                                     [Button.inline(
                                         "• حصل على بوت ايجيثون بنفسك •", data="deployme")]
                             ])
...
async def settings(event):
    await event.edit("** ᥀︙هنا ستجد رابط ايجيثون ورابط التنصيب ايضا**:",
                     buttons=[
                         [Button.url("رابط السورس", url="https://github.com/EG-THON/Egypack")],
                         [Button.url("رابط التنصيب", url="https://heroku.com/deploy?template=https://github.com/EG-THON/Egypack")],
                         [Button.url("قناه السورس",
                                     url="https://t.me/Egython")]
                     ])
...
async def settings(event):
    if event.sender_id == OWNER_ID:
        ...
        await tgbot.send_message(event.chat_id,
                                 ...
                                 buttons=[
                                     [Button.inline(
                                         "• لنظام الحمايه • ", data="pmbot")],
                                     [Button.inline(
                                         "• لتغير الايڤ وغيرها • ⁦⁩", data="custom")]
                                 ])
    else:
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @Egython", alert=True)
...
async def pmbot(event):
    if event.sender_id == OWNER_ID:
        await event.delete()
        await tgbot.send_message(event.chat_id,
                                 "** ᥀︙اختر احد اختيارات نظام الحمايه: **",
                                 buttons=[
                                     [Button.inline("• تفعيل | تعطيل •", data="onoff"), Button.inline(
                                         "• رسالة البدء •", data="cmssg")],
                                     [Button.inline("• صورة البوت •", data="btpic")],
                                     [Button.inline("• رجوع •", data="settings")]
...
async def pmbot(event):
    if event.sender_id == OWNER_ID:
        await event.delete()
        await tgbot.send_message(event.chat_id,
                                 f"** ᥀︙اختر تفعيل او تعطيل لنظام الحمايه **.\n **- نظام الحمايه الان:** `{LOAD_MYBOT}`",
                                 buttons=[
                                     [Button.inline("• للتفعيل •", data="enable"), Button.inline(
                                         "• للتعطيل •", data="disable")],
                                     [Button.inline("• رجوع •", data="pmbot")]
...
async def dissable(event):
    if event.sender_id == OWNER_ID:
        ...
        mssg = "** ᥀︙تم تعطيل نظام الحمايه في الخاص.**"
        await event.delete()
        await tgbot.send_message(event.chat_id, mssg)
    else:
        await event.answer(" انت لا تستطيع استخدام البوت.", alert=True)
...
async def custommm(event):
    await event.edit("** ᥀︙اختر احد الخيارات الخاصه بتغير الاتي: **",
                     buttons=[
                         [Button.inline("• الايڤ •", data="alive_cus")],
                         [Button.inline("• حماية الخاص •", data="pm_cus")],
                         [Button.inline("• للرجوع •", data="settings")]
                     ]
                     )
# fmt: off
...
async def alv(event):
    await event.edit("** ᥀︙اختر احد الخيارات الخاصه بالايڤ:**",
                    buttons=[
                        [Button.inline("• لتغير نص الايڤ •", data="alv_txt")],
                        [Button.inline("• لتغير صورة الايڤ •", data="alv_pic")],
                        [Button.inline("• للرجوع •", data="custom")]
                    ])
...
async def alv(event):
    await event.edit("** ᥀︙اختر احد الخيارات الاتيه الخاصه بحماية الخاص:**",
                    buttons=[
                        [Button.inline("• لتغير رساله الحمايه •", data="pm_txt")],
                        [Button.inline("• لتغير صوره الحمايه •", data="pm_pic")],
                        [Button.inline("• للرجوع •", data="custom")]
                    ])
...
async def alv_pic(event):
    if event.sender_id == OWNER_ID:
        ...
        async with event.client.conversation(OWNER_ID) as conv:
            await conv.send_message(" ᥀︙ارسل /cancel ❌.")
            response = await conv.get_response()
            try:
                themssg=response.message.message
                if themssg == "/cancel":
